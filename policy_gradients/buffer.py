...
class Experience:
    ...
    # Teacher reverse-KL pre-computation cache: when the (alpha=1, lam>0) trainer needs entries-aware
    # teacher conditioning (e.g. OPSD's answer-conditioned self-teacher, hint-writer's hint, PRM's
    # per-step rewighting), the harness computes log pi_T(y_t | y_<t) at rollout time (when entries are
    # still in scope), caches it here, and reads it back per micro-batch — avoiding both (a) a redundant
    # frozen-teacher forward per micro-batch and (b) the need to thread `entries` through the replay
    # buffer. None for any teacher whose token_logprobs is entries-agnostic (plain same_family OPD, etc.).
    teacher_logprobs: torch.Tensor | None = None
    # PRM-reweighted OPSD (prms-as-teachers variant (c)): per-token process-importance weight w_t that
    # reweights the teacher reverse-KL term. Like teacher_logprobs it depends on entries (the answer is
    # needed for the answer-info-gain signal), so it is computed at rollout time and cached here. None
    # unless cfg.prm_reweight is set; per-token, mean ~1 over a sequence's action tokens.
    prm_weights: torch.Tensor | None = None
